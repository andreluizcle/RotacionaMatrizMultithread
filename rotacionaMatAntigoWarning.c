 ...
 *
 * Estratégia de rotação (sem matriz auxiliar):
 ...
 *  - Tiling (blocos 16x16) na transposta para aproveitamento de cache L1.
 *  - Distribuição intercalada de linhas para balanceamento de carga.
 *  - Variáveis de controle de laço com 'register' para acesso rápido.
 *  - Alocação dinâmica em única etapa (malloc contíguo).
 *  - Barreira POSIX (pthread_barrier_t) entre transposta e espelhamento.
 *  - Medição de tempo por thread com clock_gettime(CLOCK_MONOTONIC).
 *
 * Compilação:
...
/** Tamanho do bloco de cache tile (linhas e colunas). */
#define TILE 16
...
{
    register int bi, bj;     /* iteradores de bloco tile          */
    register int i,  j;      /* iteradores dentro do bloco        */
    register int iMax, jMax; /* limites do bloco corrente         */
    register int tmp;        /* variável temporária para a troca  */
    register int jIni;       /* coluna inicial dentro do bloco    */

    /*
     * Itera pelas linhas do triângulo superior de forma intercalada:
     * thread 0 → linhas 0, T, 2T, ...
     * thread 1 → linhas 1, T+1, 2T+1, ...
     *
     * O tiling é aplicado apenas no eixo j (colunas) para aproveitar
     * cache. O eixo i é controlado pela intercalação entre threads.
     */
    for (i = id; i < N; i += T) {
        /* Percorre as colunas do triângulo superior em blocos TILE */
        for (bj = i + 1; bj < N; bj += TILE) {
            jMax = bj + TILE;
            if (jMax > N) jMax = N;
